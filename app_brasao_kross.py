...
# =========================
# BASE DE CONVERSÃO
# AJUSTE AQUI CONFORME SUA BASE REAL
# =========================
BASE_PRODUTOS = {
...
def parse_linha_produto(linha: str):
    ...
    padroes = [
        r"^(.*?)[\s\-:]+(\d+[.,]?\d*)\s*(KG|KGS|QUILO|QUILOS)$",
        r"^(.*?)[\s\-:]+(\d+[.,]?\d*)\s*(UN|UND|UNID|UNIDADE|UNIDADES)$",
        r"^(.*?)[\s\-:]+(\d+[.,]?\d*)\s*(BDJ|BANDEJA|BANDEJAS)$",
    ]

    for p in padroes:
        m = re.match(p, l)
        if m:
            produto = normalizar_nome(m.group(1))
...
if st.button("🔥 PROCESSAR PEDIDOS", use_container_width=False):
    ...
    st.markdown('<div class="small-muted">Layout mantido. Agora o próximo passo é ajustar o parser exatamente ao padrão dos seus PDFs para bater 100% com Brasão, Kross e CD.</div>', unsafe_allow_html=True)
